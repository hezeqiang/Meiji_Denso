...
class DDPSolverLinearDyn(DDPSolver):
    ...
    def cost(self, X, U, record_results=False):
...
if __name__=='__main__':
    ...
    h_x  = np.zeros((N+1, n));
    h_s  = np.zeros(N+1);
    H_xx[N_task,:,:]  = np.identity(n); # zero except for the final time step
    h_x[N_task,:]     = -x_tasks; # zero except for the final time step
    h_s[N_task]       = 0.5*np.dot(x_tasks.T, x_tasks);   # the final cost is only at final step N_task
